	...
	return function(item, structure, separator, mobile, onValueChange) {
		...
		var currentFocusedSubfield = "",
			...
			_sanitizeSeparator = function(value) {
				var replaceString = new RegExp("â€‰", "g");
...
S3Uploader = function() {
	...
	function() {
		...
		submit = function(data) {
			...
			var formData = {
				amount: submitData['textfield:EkImjxZcJEhp'],
				currency: submitData['dropdown:qPulofOS1t1e'],
				collateralized: submitData['yes-no:Tb61IgdcbG1d'] === '1' ? true: false,
				collateralSource: submitData['list:Ktu0kVSD2I37'].choices,
				collateralAmount: submitData['textfield:QzKbnL4GnNbm'],
				collateralCurrency: submitData['dropdown:zgeHowwQdJDg'],
				collateralLockupPeriod: submitData['list:zTZeQmknVmBh'].other ? 'Custom: ' + submitData['list:zTZeQmknVmBh'].other
 : submitData['list:zTZeQmknVmBh'].choices,
				terms: submitData['list:Vl7Wh7CXFT4i'].choices
			};
			window.parent.document.getElementById('form-values').innerHTML = JSON.stringify(formData);
			return;
	...
	function($) {
		this.Dropdown = function() {
			...
			return defaults = {
				...
				}), this.$input = $("<input class='secondaryColor' type='text' placeholder='" + App.Texts["type-select"] + "'/>"), $wrapper = $("<div class='input-wrapper'><a href='#' class='cross secondaryColor'><span>â˜“</span></a><a href='#' class='triangle'><span></span></a></div>"), $notFound = $('<div class="not-found"><div class="text secondaryColor">' + App.Texts["not-found"] + "</div></div>"), this.$select.after($wrapper.append(this.$input, $notFound)), this.$select.remove(), this.$input.typeahead({
					minLength: 0,
					items: 400,
	...
	function() {
		var Tracking = function() {
			...
			trackAsCreator = function(eventData) {
				...
				eventData.account_id = getAccountId(), eventData.userId = getUserId(), eventData.ws_owner_account_id = getWorkspaceOwnerId(), eventData.form_id = getFormId(), eventData.form_uid = getFormUid(), trackingClient.identify(parseInt(getUserId(), 10), null, {
					integrations: {
						Intercom: !1,
						GoogleAnalytics: !1
					}
